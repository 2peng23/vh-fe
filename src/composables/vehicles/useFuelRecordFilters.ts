...
export function useFuelRecordFilters(
  rowsPage: Ref<number>,
  reload: () => void | Promise<void>,
) {
  const openRequest = ref(0);
  const range = ref<RecordRange>("all");
  const sortBy = ref("fuel_date");
  const sortDirection = ref<SortDirection>("desc");
  const filters = reactive({ recordId: "", from: "", to: "", recordedBy: "" });

  function clear() {
    Object.assign(filters, { recordId: "", from: "", to: "", recordedBy: "" });
    range.value = "all";
  }
  ...
  function params() {
    return {
      ...
      recorded_by: filters.recordedBy || undefined,
